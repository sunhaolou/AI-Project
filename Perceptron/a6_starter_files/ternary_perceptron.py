...
def student_name():
    return "Haolou Sun"  # Replace with your own name.
...
class TernaryPerceptron:
    ...
    def classify(self, x_vector):
        ...
        for i in range(len(self.W)):
            predict.append(self.W[i][-1])
            for j, element in enumerate(x_vector):
                predict[i] += self.W[i][j] * element
        return self.argmax(predict)
    ...
    def train_with_one_example(self, x_vector, y):
        ...
        # ADD YOUR CODE HERE
        check = False
        predict = self.classify(x_vector)
        if predict != y:
            check = True
            for i, element in enumerate(x_vector):
                self.W[y][i] += self.alpha * element
                self.W[predict][i] -= self.alpha * element
            self.W[y][-1] += self.alpha
            self.W[predict][-1] -= self.alpha
        return check
